...
use openvaf_transformations::{BuildPDG, CalculateDataDependence, CfgVisitor, ForwardSlice, RemoveDeadLocals, Simplify, SimplifyBranches, Visit, Strip};
use std::array;
...
impl CfgVisitor<GeneralOsdiCall> for FindTaintedStmnts {
    fn visit_input(&mut self, input: &GeneralOsdiInput, loc: LocationId) {
        match input {
...
pub fn divide_analog_block(mir: &Mir<GeneralOsdiCall>) {
    ...
    init_cfg.run_pass(Strip{ retain: &init_locations, locations: &locations });
    ...
    temp_update_cfg.run_pass(Strip{ retain: &temp_update_locations, locations: &locations });
    ...
    load_cfg.run_pass(Strip{ retain: &load_relevant_locations, locations: &locations });
    // todo remove noise
