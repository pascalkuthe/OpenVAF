...
use crate::ast_lowering::error::Error::{
    DerivativeNotAllowed, EmptyBranchAccess, NotAllowedInConstantContext, TooManyBranchAccessArgs,
    UnexpectedToken,
};
use crate::ast_lowering::error::NonConstantExpression;
...
impl<'lt> StatementExpressionFolder<'lt> {
        ...
        match parameters {
            [branch] => {
                ...
                let branch_access = ast::BranchAccess::BranchOrNodePotential(
                    Self::reinterpret_expression_as_identifier(expr_node, base)?.clone(),
                );
                self.branch_resolver
                    .resolve_branch_access(base, &Node::new(branch_access, expr_node.span))
            }
            [net1, net2] => {
                ...
                let span = span.extend(expr_node.span);
            ...
            [] => {
                base.error(EmptyBranchAccess {
                    nature: nature_indent.name,
                    span: nature_indent.span,
                });
                None
            }
            [_, _, unexpected, ..] => {
                base.error(TooManyBranchAccessArgs {
                    nature: nature_indent.name,
                    span: base.ast[*unexpected]
                        .span
                        .extend(base.ast[*parameters.last().unwrap()].span),
                });
                None
            }
        }
    ...
    fn reinterpret_expression_as_identifier<'fold>(
            ...
            expression.contents
        {
            ...
            base.error(UnexpectedToken(expression.span));
            None
        }
    }
}
    ...
    fn fold(
        ...
        let res = match expression.contents {
            ast::Expression::Primary(ast::Primary::BranchAccess(ref nature, ref branch_access)) => {
                ...
                let nature =
                    BranchResolver::resolve_discipline_access(&mut *base, nature, discipline)?;
            ...
            ast::Expression::Primary(ast::Primary::DerivativeByBranch(
                ...
                let derive_by = match (&branch_access.contents, nature) {
                    (BranchAccess::BranchOrNodePotential(name), nature) => {
                        ...
                        resolve_hierarchical!(base; name as
                            Branch(id) => {
                                ...
                                let discipline_access = BranchResolver::resolve_discipline_access(base, nature, discipline)?;
                    ...
                    (BranchAccess::Implicit(branch), nature) => {
                        ...
                        let disciplines_access =
                            BranchResolver::resolve_discipline_access(base, nature, discipline)?;
                ...
                resolve_hierarchical! {base; ident as
                    Variable(vid) => {
                        if self.state.contains(VerilogContext::CONSTANT){
                            base.error(NotAllowedInConstantContext(NonConstantExpression::VariableReference,expression.span));
                        }else {
            ...
            ast::Expression::Primary(ast::Primary::FunctionCall(ref ident, ref parameters)) => {
                resolve! { base; ident as
                    ...
                    Nature(nature) => {
                        let (branch_access,discipline) = self.reinterpret_function_parameters_as_branch_access(*ident,parameters,&mut *base)?;
