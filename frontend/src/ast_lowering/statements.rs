...
use crate::ast_lowering::branches::resolver::{BranchProbeKind, BranchResolver, NatureAccess};
...
use crate::ast_lowering::expression::{
    AllowedReferences, ConstantExpressionFolder, StatementExpressionFolder,
};
use crate::ast_lowering::lints::IgnoredDisplayTask;
use crate::ast_lowering::{ExpressionFolder, Fold, VerilogContext};
use crate::diagnostic::MultiDiagnostic;
use crate::hir::{Hir, ParameterType};
use crate::hir::{Module, Statement};
use crate::ir::ast::StringParameterConstraint;
use crate::ir::hir::{Block, CaseItem, Cases, ForLoop, Parameter};
use crate::ir::hir::{DisciplineAccess, Function, FunctionArg};
use crate::ir::ids::IdRange;
use crate::ir::ids::{
    BlockId, BranchId, ExpressionId, FunctionId, ParameterId, StatementId, VariableId,
};
use crate::ir::{Node, ParameterExcludeConstraint, Spanned};
use crate::ir::{ParameterRangeConstraint, ParameterRangeConstraintBound};
                ...
                if let Some(scope) = &self.base.ast[id].scope {
                    ...
                    for decl in scope.symbols.values().copied() {
                        match decl {
                            ...
                            SymbolDeclaration::PortBranch(_) => unreachable_unchecked!("Functions can't be declared inside blocks so the parser won't ever place this here"),
