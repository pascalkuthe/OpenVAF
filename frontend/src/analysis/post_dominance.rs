...
impl ControlFlowGraph {
    ...
    pub fn post_dominators(&self) -> IPDOM {
        ...
        let mut res: IndexVec<BasicBlockId, BasicBlockId> =
            index_vec![self.start();self.blocks.len()];
...
mod print {
    ...
    pub struct IpdomTree<'lt>(&'lt ControlFlowGraph, &'lt IPDOM);

    impl ControlFlowGraph {
        pub fn render_post_dominators<W: Write>(&self, write: &mut W, ipdom: &IPDOM) {
            dot::render(&IpdomTree(self, ipdom), write).expect("Rendering failed")
        }
    }

    impl<'a> dot::Labeller<'a> for IpdomTree<'a> {
        ...
        fn edge_label(&'a self, _: &(BasicBlockId, BasicBlockId)) -> LabelText<'a> {
            LabelText::LabelStr(Cow::Borrowed("post dominates"))
        }
    }

    impl<'a> dot::GraphWalk<'a> for IpdomTree<'a> {
        type Node = BasicBlockId;
        type Edge = (BasicBlockId, BasicBlockId);
