...
use openvaf_data_structures::index_vec::{index_vec, IndexSlice};
use openvaf_ir::ids::{CallArg, NodeId};
use openvaf_ir::{Print, StopTask};
    ...
    <F as CfgFunctions>::I: From<PartialTimeDerivative>,
{
    ...
    fn fderivative<X: CfgFunctions>(
        ...
        RValue::BinaryOperation(
            ...
            Operand {
                span,
                contents: outer_derivative,
            },
            Operand {
                contents: inner_derivative,
                span,
            },
        )
        .into()
    ...
    F: CfgFunctions + From<Self>,
{
    ...
    fn fderivative<X: CfgFunctions>(
        ...
        let lhs = dmag.map(|dmag| {
            ...
            RValue::Call(
                AcStimulus(self.0).into(),
                index_vec![dmag, args[1].clone()],
                span,
            )
        });
    ...
    F: CfgFunctions,
{
    ...
    fn fderivative<X: CfgFunctions>(
        ...
        let derivative = self
            .voltage
            .derivative(ad.unknown, ad.ad.mir)
            .into_option()?;

        Some(RValue::Use(Spanned {
            span,
            contents: OperandData::Constant(derivative),
        }))
    }
}
