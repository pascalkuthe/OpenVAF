...
impl UseDefGraph {
    pub fn inverse(&self) -> DefUserGraph {
        let mut def_use_chains = SparseBitSetMatrix::new_empty(
            self.use_def_chains.y_len_idx(),
            self.use_def_chains.x_len_idx(),
        );
...
impl<'a> ReachingDefinitionsAnalysis<'a> {
    pub fn new<C: CallType>(cfg: &ControlFlowGraph<C>, locations: &'a InternedLocations) -> Self {
