...
///
/// Note that [`linkme`](https://crates.io/crates/linkme) currently has to be a **directy dependency** (in your Cargo.toml.old) of every crate that calls this macro
///
/// # Arguments
///
...
/// # use openvaf_diagnostics::lints::LintLevel;
...
///     assert_eq!(foo.data().display_id,"openvaf-diagnostics::foo");
///     assert_eq!(foo.lvl().0,LintLevel::Warn);
///     assert_eq!(bar.data().display_id,"openvaf-diagnostics::bar");
...
session_data!(static linter_lock: RwLock<Linter> =  RwLock::new(Linter::default()) );

fn with_linter<T>(f: impl FnOnce(&Linter) -> T) -> T {
    with_linter_lock(|linter_lock| f(&linter_lock.read()))
}

fn with_linter_mut<T>(f: impl FnOnce(&mut Linter) -> T) -> T {
    with_linter_lock(|linter_lock| f(&mut linter_lock.write()))
}
