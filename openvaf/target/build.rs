...
fn gen_msvcrt_importlib(sh: &Shell, arch: &str, target: &str, machine: &str, check: bool) {
    let def_dir =
        stdx::project_root().join("openvaf").join("target").join("ucrt").join("defs").join(arch);
    ...
    for src_file in std::fs::read_dir(def_dir).expect("access to openvaf/target/ucrt/defs") {
        let src_file = src_file.expect("access to files in openvaf/target/ucrts/defs").path();
