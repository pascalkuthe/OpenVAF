...
impl<F: Forest> std::fmt::Debug for Path<F> {
...
impl<F: Forest> Default for Path<F> {
    fn default() -> Self {
        Self { size: 0, node: [Node(0); MAX_PATH], entry: [0; MAX_PATH], unused: PhantomData }
    }
}
...
impl<F: Forest> Path<F> {
    ...
    pub fn find(
        ...
        for level in 0.. {
            ...
            match pool[node] {
                ...
                NodeData::Free { .. } => {
                    panic!("Free {} reached from {}", node, root)
                }
            }
        }
        unreachable!();
    ...
    //     /// Move path to the last entry of the tree starting at `root` and return it.
    //     pub fn goto_last(&mut self, pool: &NodePool<F>) {
    //         self.goto_subtree_last(0, self.node[0], pool);
    //     }
    ...
    pub fn advance_to_insert_pos(
        ...
        loop {
            ...
            if let Some(node) = tmp.next_node(leaf_level, pool) {
                let (keys, vals) = pool[node].unwrap_leaf();
                match comp.cmp(keys[0], key) {
                    Ordering::Less => {
                        *self = tmp;
                        continue;
                    }
    ...
    #[inline]
    /// get the root node and point the path at the first entry of the node.
    ...
    ///// This function does not check for duplicate keys. Use `find` or similar for that.
    /////
    ///// Afterwards the path is moved so another node can be inserted
    /////
    /////
    ///// Returns the new root node.
    //pub fn insert_and_move(
    //    &mut self,
    //    key: F::Key,
    //    value: F::Value,
    //    pool: &mut NodePool<F>,
    //) -> Node {
    //    self.insert(key, value, pool);
    //    self.entry[self.size - 1] += 1;
    //    self.node[0]
    //}
    ...
    fn heal_level(&mut self, status: Removed, level: usize, pool: &mut NodePool<F>) -> bool {
        match status {
            ...
            Removed::Rightmost => {
                ...
                debug_assert_eq!(usize::from(self.entry[level]), pool[self.node[level]].entries());
    ...
    fn update_right_crit_key(&self, level: usize, crit_key: F::Key, pool: &mut NodePool<F>) {
        let bl = self.right_sibling_branch_level(level, pool).expect("No right sibling exists");
