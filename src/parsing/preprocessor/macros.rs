...
impl<'lt> Preprocessor<'lt> {
    pub(super) fn process_declaration(&mut self, node: ParseTreeNode<'lt>) -> PreprocessorResult {
        ...
        if let Some(overwritten) = self.macros.insert(name, MACRO { args, text }) {
            warn!("{}", error_message::<Rule>("Declared macro is being overwritten here", span))
        }
        Ok(())
    }
    ...
    pub(super) fn process_reference(&mut self, node: ParseTreeNode<'lt>) -> PreprocessorResult {
        ...
        self.calling_macros.push(
            name.clone());//macro is being processed it may not be called now
        ...
        //according to standard the macro body needs to be processed when its referenced and not when its declared (to ensure the right references are used
    ...
    fn process_macro_body_line(&mut self, arg: ParseTreeNode<'lt>) -> PreprocessorResult {
        for arg_token in arg.into_inner() {
            match arg_token.as_rule() {
                ...
                Rule::MACRO_ARGUMENT => {
                    let text = self.process_macro_body_line(arg_token)?;
                }
    ...
    pub(super) fn process_macro_condition(&mut self, node: ParseTreeNode<'lt>) -> PreprocessorResult {
        ...
        if self.evaluate_condition(if_ndef, description.next().unwrap(), description.next().unwrap())? {
            return Ok(());
        }
    ...
    fn evaluate_condition(&mut self, if_ndef: bool, ident_node: ParseTreeNode<'lt>, body: ParseTreeNode<'lt>) -> PreprocessorResult<bool> {
