...
pub fn run(sess: &mut TestSession) -> Result<()> {
    if let Some(model) = sess.test_case().unwrap().model {
        model.read_files_to_vfs(sess)?;
    }
    let db = sess.db();
    let Preprocess { diagnostics, .. } = db.preprocess(db.root_file);
    sess.assert_empty_diagnostics(&diagnostics)?;
    let res = db.parse(db.root_file);
    sess.assert_empty_diagnostics(&res.errors())?;
    let x = db.item_tree(db.root_file);
    Ok(())
}
...
//         assert_eq!(ast[*branchid].contents.lo_net.as_ref().unwrap().names[0].as_str(), net2_name);
//     } else {
...
//         assert_eq!(net.discipline.map(|discipline| discipline.as_str()).as_deref(), discipline);
