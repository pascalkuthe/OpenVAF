...
use eyre::bail;
use eyre::WrapErr;
use data_structures::iter;
use data_structures::iter::Itertools;
use diagnostics::lints::Linter;
use diagnostics::{ExpansionPrinter, MultiDiagnostic};
use middle::const_fold::ConstantPropagation;
use middle::{LocalKind, VariableLocalKind};
use pass::program_dependence::control_dependence::BuildControlDependenceGraph;
use pass::program_dependence::data_dependence::BuildUseDefGraph;
use pass::program_dependence::BuildPDG;
use pass::{
...
fn run(sess: &TestSession) -> Result<()> {
    ...
    for (id, module) in mir.modules.iter_enumerated() {
        ...
        if !malformations.is_empty() {
            mir.print_to_file_with_shared(sess.log_file("invalid.mir")?, id, &cfg)
                .wrap_err("Failed to print MIR")?;
            bail!("Invalid CFG produced by SimplifyBranches:\n{}", malformations)
        }
        ...
        if !malformations.is_empty() {
            mir.print_to_file_with_shared(sess.log_file("invalid.mir")?, id, &cfg)
                .wrap_err("Failed to print MIR")?;
            bail!("Invalid CFG produced by RemoveDeadLocals:\n{}", malformations)
        }
        ...
        for (local, name) in locals {
            ...
            if out_set.is_empty() {
                bail!("No assignments to '{}' are valid at the end of the analog block!", name)
            }
            ...
            sliced_cfg.modify(Strip { retain: &retain, locations: &locations });
            ...
            if !malformations.is_empty() {
                use std::io::Write;
                write!(File::create(sess.log_file("retained_locations")?)?, " {:#?}", retain)?;
                ...
                mir.print_to_file_with_shared(sess.log_file("after_optimization.mir")?, id, &cfg)
                    .wrap_err("Failed to print MIR")?;
                bail!("Invalid CFG produced for backward slice of {}:\n{}", name, malformations)
            }
