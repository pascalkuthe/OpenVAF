...
use crate::framework::session::TestDataBase;
...
use crate::framework::{ProgressBar, ProgressStyle};
...
impl Test {
    ...
    pub fn dump_crash(dir: PathBuf, test_case: Option<TestCase>, content: &str, pb: &ProgressBar) {
        ...
        pb_println!(pb, "Saving crash report to '{}'", res.display());
        ...
        {
            #[cfg(feature = "cli")]
            let err = err.note("Crash report was not saved. Copy from the terminal instead");
            pb_println!(pb, "\n{}\n", err);
        }
    }
    ...
    fn run(
        ...
        let pretty_file = HierarchicalLayer::new(2)
            .with_ansi(false)
        ...
        #[cfg(feature = "cli")]
        let collector = tracing_subscriber::registry().with(filter).with(pretty_file);

        #[cfg(not(feature = "cli"))]
        let collector = tracing_subscriber::registry().with(pretty_file);
        ...
        tracing::subscriber::with_default(collector, || {
            ...
            let res = std::panic::catch_unwind(move || {
                let root_file = self
                    .root_file
                    .map(|f| f(test_case, test_case.and_then(|c| c.model).or(self.model)));
                let mut session =
                    TestSession::new(config, self, test_case, root_file.as_deref(), pb.clone());
...
impl Display for TestSummary {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let prefix = match self.result {
            TestResult::Passed(_) => Style::new().italic().green().for_stdout().apply_to("passed"),
...
pub fn run_tests(config: &Config) -> Result<bool> {
    ...
    let base = format!(
        ...
        Style::new().italic().yellow().apply_to(format!("v{}", VERSION))
    );
    println!("{:->width$}\n\n", base, width = min(Term::stdout().size().1 as usize, 70));
    ...
    let TestsToRun { max_test_len, max_testcase_len, test_cnt, tests } =
    ...
    for (test, test_cases) in tests {
        if let Some(cases_to_run) = test_cases {
            if cases_to_run.len() == 0 {
                #[cfg(feature = "cli")]
                pb.set_message(
                    TestTitle { test, case: None, max_test_len, max_testcase_len }.to_string(),
                );
                #[cfg(feature = "cli")]
                pb.reset_elapsed();
                ...
                pb_println!(
                    pb,
                    "{}",
                    TestSummary { test, case: None, max_test_len, max_testcase_len, result }
                );
                #[cfg(feature = "cli")]
                pb.inc(1);
            } else {
                for (case, run) in cases_to_run.iter() {
                    if *run {
                        #[cfg(feature = "cli")]
                        pb.set_message(
                            TestTitle { test, case: Some(*case), max_test_len, max_testcase_len }
                                .to_string(),
                        );
                        #[cfg(feature = "cli")]
                        pb.reset_elapsed();
