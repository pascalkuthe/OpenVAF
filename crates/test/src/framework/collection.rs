...
impl<'a> TestInitInfo<'a> {
    pub fn model_tests_cases(info: &TestInitInfo) -> Box<[TestCase]> {
        info.models.iter().map(|x| TestCase { name: x.0, model: Some(x) }).collect_vec().into()
    }

    // When more cases should be added
    pub fn model_tests_cases_vec(&self) -> Vec<TestCase> {
        self.models.iter().map(|x| TestCase { name: x.0, model: Some(x) }).collect()
    }
...
impl<'a> TestInitInfo<'a> {
    fn collect_test_cases(self) -> CollectedTest {
        ...
        let mut test_cases: Vec<_> = tests_cases.iter().copied().zip(iter::repeat(true)).collect();
...
impl CollectedTest {
    fn new_ignored(cases: &[TestCase]) -> Self {
        let cases = cases.iter().map(|x| (*x, false)).collect_vec().into_boxed_slice();
        Self { cases, ignored: true, cases_to_run: 0, longest_case_name_len: 0 }
    }

    fn new(cases: Box<[(TestCase, bool)]>, cases_filtered: bool) -> Self {
        if cases.len() == 0 {
            Self { cases, ignored: cases_filtered, cases_to_run: 1, longest_case_name_len: 0 }
        } else {
            ...
            let cases_to_run = cases
                ...
                .count();
            Self { cases, ignored: cases_to_run == 0, cases_to_run, longest_case_name_len }
        }
    }
}
...
impl TestsToRun {
    pub fn collect(config: &Config, models: &'static [Model]) -> Self {
        ...
        let tests = TESTS
            .iter()
            .map(|test| {
                let CollectedTest { cases, ignored, cases_to_run, longest_case_name_len } =
                    TestInitInfo { config, models, test }.collect_test_cases();
        ...
        max_test_len += 4;
        Self { max_test_len, max_testcase_len, test_cnt, tests }
    }
}
