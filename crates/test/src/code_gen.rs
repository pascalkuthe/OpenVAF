...
use codegen_llvm::inkwell::module::Linkage;
use codegen_llvm::inkwell::targets::{
    CodeModel, InitializationConfig, RelocMode, Target, TargetMachine,
};
use codegen_llvm::inkwell::OptimizationLevel;
use codegen_llvm::LlvmCodegen;
use diagnostics::{ExpansionPrinter, MultiDiagnostic};
use middle::cfg::{ControlFlowGraph, START_BLOCK};
use middle::const_fold::ConstantPropagation;
use middle::DefaultConversion;
use pass::{RemoveDeadLocals, Simplify, SimplifyBranches};
...
fn run(sess: &TestSession) -> Result<()> {
    ...
    for (id, module) in mir.modules.iter_enumerated() {
        ...
        codegen.ctx.builder.position_at_end(codegen.blocks[START_BLOCK]);
        ...
        codegen_ctx.module.print_to_file(sess.log_file("final.ll")?).unwrap();
    }
