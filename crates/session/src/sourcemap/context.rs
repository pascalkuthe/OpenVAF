...
fn with_syntax_interner<T, F: FnOnce(&mut SyntaxContextInterner) -> T>(f: F) -> T {
    with_syntax_interner_lock(|interner_lock| f(&mut *interner_lock.lock()))
}
session_data!(static syntax_interner_lock: Lock<SyntaxContextInterner> = Lock::new(SyntaxContextInterner::default()));
