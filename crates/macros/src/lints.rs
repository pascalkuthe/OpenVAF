...
impl Parse for Lint {
    fn parse(input: &ParseBuffer) -> Result<Self> {
        ...
        Ok(Self { visibility, name, declaration })
    }
}
...
pub(crate) fn generate_lints(lints: Lints) -> TokenStream {
    ...
    if count > u16::MAX as usize {
        panic!("At most {} lints are allowed but {} builtin lints were declared", u16::MAX, count)
    }
    for (i, Lint { visibility, name, declaration }) in lints.0.into_iter().enumerate() {
