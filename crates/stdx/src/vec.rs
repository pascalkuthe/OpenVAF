...
pub trait SliceExntesions<T> {
    ...
    fn pick_n_mut<const N: usize>(&mut self, indecies: [usize; N]) -> [&mut T; N];
}
...
impl<T> SliceExntesions<T> for [T] {
    ...
    fn pick_n_mut<const N: usize>(&mut self, indecies: [usize; N]) -> [&mut T; N] {
        ...
        for idx1 in indecies {
            ...
            for idx2 in &indecies[..idx1] {
                assert_ne!(idx1, *idx2);
            }
            for idx2 in &indecies[(idx1 + 1..)] {
                assert_ne!(idx1, *idx2);
            }
        }

        let mut res = [self.as_mut_ptr(); N];
        for (res, idx) in Iterator::zip(res.iter_mut(), indecies) {
