...
use crate::error::Error::{IllegalFinishNumber, TypeMissmatch};
use crate::error::MockType;
use crate::expression_semantic::{
    ConstantSchematicAnalysis, InliningSchemanticAnalysis, SchematicAnalysis,
};
use crate::HirToMirFold;
use openvaf_constant_fold::{constant_fold_int_expr, NoConstResolution};
use openvaf_data_structures::index_vec::IndexVec;
use openvaf_hir as hir;
use openvaf_hir::Block;
use openvaf_ir::ids::{ExpressionId as HirExpressionId, IntegerExpressionId, VariableId};
use openvaf_ir::{Attributes, Node, PrintOnFinish, Spanned};
use openvaf_mir::cfg::{BasicBlock, BasicBlockId, ControlFlowGraph, Terminator};
use openvaf_mir::{ComparisonOperator, ExpressionId, IntegerBinaryOperator, Statement};
use openvaf_mir::{IntegerExpression, VariableType};
            ...
            match self.hir[statement].contents {
                hir::Statement::StopTask(kind, print) => {
                    let print = print
                        ...
                        .and_then(|expr| {
                            match constant_fold_int_expr(
                                &mut self.mir,
                                expr,
                                &mut NoConstResolution,
                            )
