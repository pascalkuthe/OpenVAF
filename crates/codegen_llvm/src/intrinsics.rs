...
impl Intrinsic {
    ...
    pub fn fn_type(self, ctx: &Context) -> FunctionType<'_> {
        match self {
            ...
            Self::ATan2 | Self::Hypot | Self::Pow | Self::FloatMin | Self::FloatMax => {
                ctx.f64_type().fn_type(&[ctx.f64_type().into(), ctx.f64_type().into()], false)
            }

            Self::IntAbs => {
                ctx.i64_type().fn_type(&[ctx.i64_type().into(), ctx.bool_type().into()], false)
            }

            Self::IntMin | Self::IntMax => {
                ctx.i64_type().fn_type(&[ctx.i64_type().into(), ctx.i64_type().into()], false)
            }
            Self::StringCmp => {
