...
use ir::ids::{BranchId, CallArg};
use middle::cfg::{BasicBlock, ControlFlowGraph, PhiData, TerminatorKind};
use middle::{
    ...
    C: CallTypeCodeGen<'lt, 'c>,
{
    pub fn new_with(
        ...
        let blocks = cfg
            .blocks
            .indices()
            .map(|block| ctx.context.append_basic_block(function, &format!("Block {}", block)))
            .collect();

        Self { ctx, blocks, cfg, local_values: IndexVec::new(), function, call_type_data }
    }
    ...
    CfgCodegen<'lt, 'a, 'c, C::CodeGenData, A, C>
{
    ...
    pub fn write_local(&mut self, local: Local, val: BasicValueEnum<'c>) {
        match self.local_values[local] {
            ...
            LocalValue::Value(old) => {
                unreachable!("Local {} was written twice (first: {:?} then {:?})", local, old, val)
            }
    ...
    pub fn real_comparison(
        ...
        self.ctx
            .builder
            .build_float_compare(pred, lhs.into_float_value(), rhs.into_float_value(), "float_cmp")
            .as_basic_value_enum()
    }
    ...
    pub fn string_comparison(
        ...
        let res = self.ctx.build_intrinsic_call(Intrinsic::StringCmp, &[lhs, rhs]).into_int_value();
        let predicate = if invert { IntPredicate::NE } else { IntPredicate::EQ };

        self.ctx
            .builder
            .build_int_compare(predicate, res, res.get_type().const_zero(), "string_comparison")
            .as_basic_value_enum()
    }
    ...
    pub fn build_bb(&mut self, bb: BasicBlock) {
        ...
        match bb.terminator().kind {
            TerminatorKind::Goto(dst) => {
                self.ctx.builder.build_unconditional_branch(self.blocks[dst]);
            }
            TerminatorKind::Split { ref condition, true_block, false_block, .. } => {
    ...
    pub fn build_phi(&mut self, phi: &PhiData) {
        ...
        let incoming: Vec<_> = phi
            ...
            .collect();
        let incoming: Vec<_> =
            incoming.iter().map(|(val, block)| (val as &dyn BasicValue<'c>, *block)).collect();
