...
use data_structures::index_vec::{define_index_type, Idx};
use std::fmt::Debug;
use std::iter;
use std::ops::Range;
...
///
/// * `$sub_container` - the field of the ir which contains the [Arena](data_structures::index_vec::IndexVec) this ID indexes
///
...
macro_rules! impl_id_type {
    ...
    ($name:ident in $container:ty => $sub_container:ident as $type:ty where $($generics:tt)*) => {
        ...
        impl$($generics)* ::std::ops::Index<::core::ops::Range<$name>> for $container {
            type Output = data_structures::index_vec::IndexSlice<$name, [$type]>;
