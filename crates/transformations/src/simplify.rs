...
use openvaf_middle::cfg::{
    BasicBlock, BasicBlockData, CfgPass, ControlFlowGraph, Terminator, TerminatorKind, START_BLOCK,
};
...
impl<C: CallType> CfgPass<'_, C> for Simplify {
    type Result = ();
    ...
    fn run(&mut self) {
        ...
        loop {
            ...
            for bb in self.cfg.blocks.indices() {
                ...
                let mut terminator = self.cfg[bb]
                    .terminator
                    .take()
                ...
                loop {
                    let dst = match terminator.kind {
                        ...
                        // TODO difference between infinite loop and dead code result (not that important because the former is just bullshit we shoudl just shoot that down in some pass befhorehand)
                        TerminatorKind::Split {
                            true_block,
                            false_block,
                            loop_head: true,
                ...
                let statements_to_merge = merged_blocks
                    .iter()
                    .map(|&i| self.cfg.blocks[i].statements.len())
                    .sum();
    ...
    for data in cfg.blocks.iter_mut() {
        for phi in data.phi_statements.iter_mut() {
            phi.sources = phi
                .sources
                .iter()
                .map(|(block, local)| (replacements[*block], *local))
                .collect();
        }
