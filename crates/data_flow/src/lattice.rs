...
impl<I: From<usize> + Into<usize>, T: JoinSemiLattice> JoinSemiLattice for TiVec<I, T> {
    fn join(&mut self, other: &Self) -> bool {
...
impl<I: From<usize> + Into<usize>, T: MeetSemiLattice> MeetSemiLattice for TiVec<I, T> {
    fn meet(&mut self, other: &Self) -> bool {
        debug_assert_eq!(self.len(), other.len());
...
impl<T: From<usize> + Into<usize> + Copy + PartialEq + Debug> JoinSemiLattice for BitSet<T> {
    fn join(&mut self, other: &Self) -> bool {
        self.union(other)
    }
}

impl<T: From<usize> + Into<usize> + Copy + PartialEq + Debug> MeetSemiLattice for BitSet<T> {
    fn meet(&mut self, other: &Self) -> bool {
...
impl<K: Hash + Eq + From<usize> + Into<usize> + Copy + PartialEq + Debug, V: PartialEq + Clone>
    JoinSemiLattice for SparseFlatSetMap<K, V>
{
    fn join(&mut self, other: &Self) -> bool {
        // Join top sets
        ...
        self.element_sets.retain(|key, val| match other.element_sets.get(key) {
...
impl<T: PartialEq> From<FlatSet<T>> for Option<T> {
    fn from(set: FlatSet<T>) -> Option<T> {
        set.into_option()
    }
}
