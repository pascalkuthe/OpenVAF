...
use crate::diagnostics::MacroOverwritten;
...
impl<'a> Processor<'a> {
    ...
    pub(crate) fn include_file(
                ...
                if let Some(path) = dir.join(path) {
                    ...
                    match self.sources.file_text(file) {
                        ...
                        Err(FileReadError::InvalidTextFormat) => {
                            return Err(FileReadError::InvalidTextFormat)
                        }
        ...
        let (src, file) = found.ok_or(FileReadError::Io)?;
    ...
    pub(crate) fn define_macro(
        ...
        if let Some(old) = self.macros.insert(name, def) {
            diagnostics.push(
                MacroOverwritten { old: old.head_span(), new: span, name: name.to_owned() }.into(),
            )
        }
    }
    ...
    fn process_macro_token(
        ...
        token: &ParsedToken<'a>,
        span: CtxSpan,
        args: &IndexSlice<MacroArg, [(TokenStream, FileSpan)]>,
        dst: &mut TokenStream,
        errors: &mut Diagnostics,
    ) {
        match *token {
            ...
            ParsedToken::ArgumentReference(arg) => {
                let (ref ts, _decl) = args[arg];
                // TODO find a way to generated macro backtraces for macro args
                // let ctx = self.source_map.add_ctx(decl, span);
                dst.extend(ts);
            }
    ...
    pub(crate) fn call_macro(
        ...
        if let Some(def) = self.macros.get(&call.name).cloned() {
            let new_args: IndexVec<_, _> = call
                ...
                .map(|(arg, decl)| {
                    ...
                    for (token, range) in arg {
                        ...
                        let span = CtxSpan { range: range - decl.start(), ctx: span.ctx };
                        self.process_macro_token(token, span, args, &mut dst, errors)
                    }
                    let decl = span.to_file_span(&self.source_map).with_range(*decl);
                    (dst, decl)
                })
                .collect();
            ...
            errors.push(MacroNotFound(call.name.to_owned(), span))
        }
    }
    ...
    pub(crate) fn process_token(&mut self, p: &mut Parser<'a, '_>, err: &mut Diagnostics) {
        match p.current() {
            RawToken::Include => {
                ...
                if let Some((file_name, range)) = parse_include(p, err) {
                    ...
                    match self.include_file(file_name, span, p.dst, err, &p.working_dir) {
                        Ok(_) => (),
                        Err(FileReadError::InvalidTextFormat) => {
                            err.push(PreprocessorDiagnostic::InvalidTextFormat(span))
                        }
                        Err(FileReadError::Io) => err.push(PreprocessorDiagnostic::IoError(span)),
                    }
                }
            }
            RawToken::MacroIf => {
            ...
            RawToken::MacroCall => {
                let (call, range) = parse_macro_call(p, err, &[], &mut self.source_map);
                let span = CtxSpan { range, ctx: p.ctx() };
