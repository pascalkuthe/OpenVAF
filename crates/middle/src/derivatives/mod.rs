...
pub use rvalue::{OuterDerivativeCacheSlot, RValueAutoDiff};
...
impl<C: CfgFunctions> ControlFlowGraph<C> {
    pub fn generate_derivatives<MC: CfgFunctions>(
        ...
        AutoDiff { cfg: self, mir, errors, forward_stmnts: IndexVec::with_capacity(128) }.run()
    }
}
        ...
        for id in postorder {
            ...
            for phi in &phis {
                    ...
                    self.cfg.derivatives.get(&phi.dst).cloned().into_iter().flatten()
                {
                    ...
                    self.cfg[id].phi_statements.push(PhiData { dst, sources, sctx: phi.sctx });
                }
            }
