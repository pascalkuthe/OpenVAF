...
impl Direction for Backward {
        ...
        A: Analysis<C>,
    {
        ...
        if start < block_data.phi_statements.len() {
            ...
            let end = Phi::new(end);
            // Handle all statements
            for (phi, phi_data) in block_data.phi_statements[start..end].iter_enumerated().rev() {
        ...
        C: CfgFunctions,
    {
        ...
        for phi in cfg.blocks[block].phi_statements.indices().rev() {
            vis.visit_phi_before_effect(
                state,
                unsafe { cfg.blocks[block].phi_statements.raw.get_unchecked_mut(phi.index()) },
                block,
                phi,
            );
            results.reconstruct_phi_effect(
                cfg,
                state,
                unsafe { cfg.blocks[block].phi_statements.raw.get_unchecked(phi.index()) },
                block,
                phi,
            );
        ...
        for stmnt in cfg.blocks[block].statements.indices().rev() {
            vis.visit_statement_before_effect(
                state,
                unsafe { cfg.blocks[block].statements.raw.get_unchecked_mut(stmnt.index()) },
                block,
                stmnt,
            );
            results.reconstruct_statement_effect(
                cfg,
                state,
                unsafe { cfg.blocks[block].statements.raw.get_unchecked(stmnt.index()) },
                block,
                stmnt,
            );
