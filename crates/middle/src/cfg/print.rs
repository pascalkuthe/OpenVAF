use std::io::Write;
use crate::{ CallType,  Mir,  LocalKind, VariableLocalKind, ModuleId};
use std::{ io};
use crate::cfg::{ControlFlowGraph, BasicBlockData};
use std::path::Path;
...
impl<C: CallType> Mir<C>{
    pub fn print_header<W: io::Write>(&self,  mut w: W) -> io::Result<()>{
        ...
        for (param,info) in self.parameters.iter_enumerated(){
            ...
            info.ty.with_info(|ty| writeln!(w,"{}param {}: {} = {};",INDENT,param, ty, info.ident))?;
        }
    ...
    pub fn print_modules<W: io::Write>(&self,  mut w: W) -> io::Result<()>{
        for module in self.modules.iter(){
            ...
            for port in module.ports.clone(){
                ...
                writeln!(w,"{0}{0}port {1}: {2} = {3:?};",INDENT, port, directions, self.ports[port].net)?;
            }
    ...
    pub fn print_modules_with_shared<W: io::Write>(&self,  mut w: W, id: ModuleId, cfg: &ControlFlowGraph<C>) -> io::Result<()>{
    ...
    pub fn print_to_file_with_shared(&self,path: impl AsRef<Path>, id: ModuleId, cfg: &ControlFlowGraph<C>) -> io::Result<()>{
        let mut file = std::fs::File::create(path.as_ref())?;
        self.print_header(&mut file)?;
        writeln!(file, "")?;
    ...
    pub fn print<A: CallType, W: io::Write>(&self, mir: &Mir<A>, mut w: W) -> io::Result<()>{
        ...
        for (local, decl) in self.locals.iter_enumerated(){
            let (decl, comment) = match decl.kind{
                LocalKind::Temporary => {
                    decl.ty.with_info(|ty|writeln!(w, "{0}{0}let {1}: {2};", INDENT, local, ty))?;
                    continue
                },
                LocalKind::Variable(var, VariableLocalKind::User) => {
                    (decl.ty.with_info(|ty|format!("{0}{0}let mut {1}: {2};",INDENT, local, ty)), format!("Corresponds to {}",mir.variables[var].ident))
                }
                LocalKind::Variable(var, VariableLocalKind::Derivative) => {
                    (decl.ty.with_info(|ty|format!("{0}{0}let mut {1}: {2};",INDENT, local, ty)), format!("Corresponds to a nth order derivative of {}",mir.variables[var].ident))
                }
                LocalKind::Branch(access, branch) =>  {
                    (format!("{0}{0}let mut {1}: real;",INDENT, local), format!("Corresponds to {}({})",access, mir.branches[branch].ident))
