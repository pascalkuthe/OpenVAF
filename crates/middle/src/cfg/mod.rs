...
use data_structures::{index_vec::define_index_type, index_vec::IndexVec};
...
impl<C: CfgFunctions> ControlFlowGraph<C> {
    ...
    pub fn new_temporary(&mut self, ty: Type) -> Local {
        self.locals.push(LocalDeclaration { ty, kind: LocalKind::Temporary })
    }
    ...
    pub fn locations(&self) -> impl Iterator<Item = Location> + '_ {
        self.blocks.iter_enumerated().flat_map(|(id, block)| {
            ...
            let stmnt_locations = block
                .statements
                .indices()
                .map(move |stmt| Location { block: id, kind: LocationKind::Statement(stmt) });
    ...
    pub fn map<X: CfgFunctions>(
        ...
        let blocks = self.blocks.into_iter().map(|bb| bb.map(conversion)).collect();
    ...
    fn map<X: CfgFunctions>(self, conversion: &mut impl CfgConversion<C, X>) -> BasicBlockData<X> {
        ...
        let terminator = self.terminator.map(|term| {
            let kind = match term.kind {
                ...
                TerminatorKind::Split { condition, true_block, false_block, loop_head } => {
        ...
        BasicBlockData { phi_statements: self.phi_statements, statements, terminator }
    }
...
pub enum TerminatorKind<C: CfgFunctions = DefaultFunctions> {
    Goto(BasicBlock),
    Split { condition: RValue<C>, true_block: BasicBlock, false_block: BasicBlock, loop_head: bool },
    End,
}
