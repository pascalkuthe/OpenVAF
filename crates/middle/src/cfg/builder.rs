...
impl<'a, C: CfgFunctions> CfgBuilder<&'a mut ControlFlowGraph<C>> {
    pub fn edit<const INIT_BRANCHES: bool, const INIT_VARS: bool>(
        ...
        let mut vars = if INIT_VARS { index_vec![None; var_cnt] } else { IndexVec::new() };
...
impl<C: CfgFunctions, CFG: CfgEdit<CfgFunctions = C>> CfgBuilder<CFG> {
    ...
    >(
        ...
        let (inserted_cfg_start, inserted_cfg_end) = if RETAIN_END {
            ...
            dst.blocks.extend(cfg.blocks.into_iter().map(|block| block.map(conversion)));
    ...
    pub fn rvalue_to_operand_in_bb(
        ...
        if let RValue::Use(op) = data.val {
            ...
            Spanned::new(OperandData::Copy(self.assign_temporary_in_bb(bb, data, sctx)), span)
        }
    }
    ...
    pub fn assign_phi_in_bb(
        ...
        self.cfg.borrow_mut().blocks[bb].phi_statements.push(PhiData { dst, sources, sctx });
        dst
    }
    ...
    pub fn assign_in_bb(
        ...
        self.cfg.borrow_mut().blocks[bb].statements.push((StmntKind::Assignment(lhs, rhs), sctx))
    }
