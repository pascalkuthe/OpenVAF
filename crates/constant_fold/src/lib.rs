...
use crate::DiamondLattice::{NotAConstant, Val};
...
use openvaf_mir::fold::integer_expressions::walk_integer_expression;
...
use openvaf_mir::fold::real_expressions::{walk_real_expression, RealBuiltInFunctionCall1pFold};
use openvaf_mir::fold::real_expressions::{
    RealBinaryOperatorFold, RealBuiltInFunctionCall2pFold, RealExprFold,
};
use openvaf_mir::fold::string_expressions::walk_string_expression;
use openvaf_mir::fold::string_expressions::StringExprFold;
use openvaf_mir::{
    ComparisonOperator, IntegerBinaryOperator, IntegerExpression, RealBinaryOperator,
    RealExpression, StringExpression,
};
use openvaf_mir::{DisciplineAccess, ExpressionId, Mir, UnaryOperator};
...
pub use crate::lattice::{DiamondLattice, ProductLattice, TypedDiamondLattice};
use crate::lints::ConstantOverflow;
pub use crate::resolver::{ConstResolver, NoConstResolution};

/// Abstraction over mutability of the mir for constant folding (see [`constant_eval_real_expr`](openvaf_mir::Mir::constant_eval_real_expr))
...
/// Implementation of constant folding as a [mir expression fold](openvaf_mir::fold).
...
/// (for example `abs(x) > 0?0:1` can not be constant folded currently)
///
///
/// # Arguments
///
...
pub fn constant_fold_expr(
    ...
    match expr {
        ExpressionId::String(expr) => constant_fold_str_expr(mir, expr, resolver).into(),
        ExpressionId::Real(expr) => constant_fold_real_expr(mir, expr, resolver).into(),
        ExpressionId::Integer(expr) => constant_fold_int_expr(mir, expr, resolver).into(),
    }
}

/// See [`constant_fold_expr`](constant_fold_real_expr)
pub fn constant_fold_real_expr(
    ...
    ConstantFold {
        fold_type: &mut MutatingConstantFold(mir),
        resolver,
        expr,
    }
    .fold_real_expr(expr)
...
/// Same as [`constant_fold_expr`] but no expressions in `self` actually change only the return value is calculated
pub fn constant_eval_expr(
    ...
    match expr {
        ExpressionId::String(expr) => constant_eval_str_expr(mir, expr, resolver).into(),
        ExpressionId::Real(expr) => constant_eval_real_expr(mir, expr, resolver).into(),
        ExpressionId::Integer(expr) => constant_eval_int_expr(mir, expr, resolver).into(),
    }
}

/// See [`constant_eval_expr`]
pub fn constant_eval_real_expr(
    ...
    ConstantFold {
        fold_type: &mut ReadingConstantFold(mir),
        resolver,
        expr,
    }
    .fold_real_expr(expr)
