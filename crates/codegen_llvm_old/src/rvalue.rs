...
use ir::{ids::CallArg, SimpleConstVal};
use middle::{COperand, CfgFunctions, RValueFold, Type};
    ...
    for CfgCodegen<'lt, 'a, 'c, C::CodeGenData, A, C>
{
    ...
    fn fold_real_arith_negate(&mut self, _: Span, arg: &COperand<C>) -> Self::T {
        ...
        self.ctx.builder.build_float_neg(arg.into_float_value(), "real_neg").as_basic_value_enum()
    }

    fn fold_bit_negate(&mut self, _: Span, arg: &COperand<C>) -> Self::T {
        let arg = self.operand(arg);
        self.ctx.builder.build_not(arg.into_int_value(), "inverse").as_basic_value_enum()
    }

    fn fold_int_arith_negate(&mut self, _: Span, arg: &COperand<C>) -> Self::T {
        let arg = self.operand(arg);
        self.ctx.builder.build_int_neg(arg.into_int_value(), "int_neg").as_basic_value_enum()
    }
    ...
    fn fold_nxor(&mut self, _: Span, lhs: &COperand<C>, rhs: &COperand<C>) -> Self::T {
        ...
        let xor = self.ctx.builder.build_xor(lhs.into_int_value(), rhs.into_int_value(), "nxor");
        self.ctx.builder.build_not(xor, "nxor").as_basic_value_enum()
    }
    ...
    fn fold_clog2(&mut self, _span: Span, arg: &COperand<C>) -> Self::T {
        ...
        let leading_zeros = self.ctx.build_intrinsic_call(Intrinsic::LeadingZeros, &[arg, flag]);
        let c64 = self.ctx.simple_constant(SimpleConstVal::Integer(64));
        self.ctx
            .builder
            .build_int_sub(c64.into_int_value(), leading_zeros.into_int_value(), "clog2")
            .as_basic_value_enum()
    }
    ...
    fn fold_int_abs(&mut self, _: Span, arg: &COperand<C>) -> Self::T {
        let arg = self.operand(arg);
        self.ctx
            .build_intrinsic_call(Intrinsic::IntAbs, &[arg, self.ctx.bool_ty().const_zero().into()])
    }
    ...
    fn fold_hypot(&mut self, _: Span, arg1: &COperand<C>, arg2: &COperand<C>) -> Self::T {
        let arg1 = self.operand(arg1);
        let arg2 = self.operand(arg2);
        self.ctx.build_intrinsic_call(Intrinsic::Hypot, &[arg1, arg2])
    }

    fn fold_real_min(&mut self, _: Span, arg1: &COperand<C>, arg2: &COperand<C>) -> Self::T {
        let arg1 = self.operand(arg1);
        let arg2 = self.operand(arg2);
        self.ctx.build_intrinsic_call(Intrinsic::FloatMin, &[arg1, arg2])
    }

    fn fold_int_min(&mut self, _: Span, arg1: &COperand<C>, arg2: &COperand<C>) -> Self::T {
        let arg1 = self.operand(arg1);
        let arg2 = self.operand(arg2);
        self.ctx.build_intrinsic_call(Intrinsic::IntMax, &[arg1, arg2])
    }

    fn fold_real_max(&mut self, _: Span, arg1: &COperand<C>, arg2: &COperand<C>) -> Self::T {
        let arg1 = self.operand(arg1);
        let arg2 = self.operand(arg2);
        self.ctx.build_intrinsic_call(Intrinsic::FloatMax, &[arg1, arg2])
    }
    ...
    fn fold_atan2(&mut self, _: Span, arg1: &COperand<C>, arg2: &COperand<C>) -> Self::T {
        let arg1 = self.operand(arg1);
        let arg2 = self.operand(arg2);
        self.ctx.build_intrinsic_call(Intrinsic::ATan2, &[arg1, arg2])
    }
    ...
    fn fold_cast(&mut self, arg: &COperand<C>, dst_ty: Type) -> Self::T {
        ...
        match (dst_ty, src_ty) {
            (Type::INT, Type::REAL) => {
                self.ctx.build_intrinsic_call(Intrinsic::FloatToIntConversion, &[src])
            }

            (Type::INT, Type::BOOL) => self
                .ctx
                .builder
                .build_int_cast(src.into_int_value(), self.ctx.integer_ty(), "implict bool to int")
                .as_basic_value_enum(),
    ...
    fn fold_select(
        ...
        self.ctx.builder.build_select(cond.into_int_value(), true_val, false_val, "select")
    }
