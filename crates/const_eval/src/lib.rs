...
impl EvalCtx<'_> {
    pub fn eval_op(&mut self, op: Op, args: &[Operand], src: i32) -> FlatSet<Const> {
        match op {
            ...
            Op::RealSub if args[0] == args[1] => FlatSet::Elem(0f64.into()),
            ...
            Op::RealMin => self.op_2(f64::min, args),
            Op::RealMax => self.op_2(f64::max, args),
            Op::RealAbs => self.op_1(f64::abs, args),
            Op::RealPow => self.op_2(f64::powf, args),
            Op::IntAbs => self.op_1(i32::abs, args),
            Op::IntMin => self.op_2(i32::min, args),
