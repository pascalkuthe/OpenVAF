...
use ast::symbol_table::{SymbolDeclaration, SymbolTable};
use ast::{Ast, HierarchicalId};
use data_structures::index_vec::{define_index_type, IndexBox, IndexVec};
...
/// If `$name` wasn't found or doesn't math any [`SymbolDeclaration`](ast::symbol_table::SymbolDeclaration)::`$declaration` the appropriate errors are added to `$self`.errors and execution continuous after the macro
///
/// # Arguments
///
/// * `$self` - reference to an ast fold (implementation of the [`Fold`](crate::Fold) trait)
///
/// * `$name` - The Identifier (type [`Ident`](session::symbols::Ident)) that should be resolved
///
/// The following three arguments can be repeated as often as necessary (like in a match block)
///
/// * `$declaration` An identifier of an [`SymbolDeclaration`](ast::symbol_table::SymbolDeclaration) variant which you wish to resolve `$name`as
///
/// * `$id` The identifier under which the ID of the resolved declaration will be available inside the associated `$block
///
/// * `$block` A block (`{`statements`}`) which will be executed when `$name` is resolved as a [`SymbolDeclaration`](ast::symbol_table::SymbolDeclaration)::`$declaration`  
///
/// # Examples
///
/// The following tries to resolve ident as a Nature
/// ```
/// use ast::symbol_table::SymbolDeclaration::Nature;
/// use ast::symbol_table::SymbolDeclaration::Discipline;
/// use ast_lowering::resolve;
/// # use ast_lowering::Fold;
/// # use ast::Ast;
/// # use diagnostics::MultiDiagnostic;
/// # use hir::Hir;
/// # use ast_lowering::name_resolution::Resolver;
/// # use session::symbols::Ident;
/// # use session::session;
///
/// # session(||{
...
/// use ast::symbol_table::SymbolDeclaration::Nature;
/// use ast::symbol_table::SymbolDeclaration::Discipline;
/// use ast_lowering::resolve_hierarchical;
/// # use ast_lowering::Fold;
/// # use ast::{Ast, HierarchicalId};
/// # use diagnostics::MultiDiagnostic;
/// # use hir::Hir;
...
impl<'lt> Resolver<'lt> {
    ...
    pub fn new(ast: &'lt Ast) -> Self {
        Self {
            ...
            ast,
            net_map: ast.nets.indices().map(|id| NodeId::from_raw_unchecked(id.raw())).collect(),
    ...
    pub fn resolve(&self, ident: &Ident) -> Result<SymbolReference> {
        for (depth, scope) in self.scope_stack.iter().rev().enumerate() {
            if let Some(&decl) = scope.get(&ident.name) {
                ...
                {
                    return Err(NonLocalAccess { ident: *ident, kind: reference.mock() });
                }
                return Ok(reference);
            }
        }
