...
use data_structures::index_vec::{index_vec, IndexSlice, IndexVec};
use ir::{ids::CallArg, DisciplineAccess, Math1, Math2, Print, SimpleType, StopTask};
...
use session::sourcemap::StringLiteral;
...
impl HirFunction for StopTask {
    ...
    fn lower<L: HirLowering>(
        ...
        Some(FunctionLoweringReturn::Void((*self).into(), IndexVec::new(), span))
    }
}
...
impl HirFunction for Discontinuity {
    ...
    fn lower<L: HirLowering>(
        ...
        FunctionLoweringReturn::Void(
            middle::functions::Discontinuity(val).into(),
            IndexVec::new(),
            span,
        )
        .into()
...
impl HirFunction for AcStimulus {
    ...
    fn lower<L: HirLowering>(
        ...
        let mag = args
            ...
            .unwrap_or(Operand { contents: Scalar(Real(1.0)).into(), span });
        ...
        FunctionLoweringReturn::Val(TyRValue {
            val: RValue::Call(
                middle::functions::AcStimulus(name).into(),
                index_vec![mag, phase],
                span,
            ),
            ty: Type::REAL,
...
impl HirFunction for Analysis {
    ...
    fn lower<L: HirLowering>(
        ...
        let names = args.iter().filter_map(|name| ctx.fold.eval_string_constant(*name)).collect();

        FunctionLoweringReturn::Val(TyRValue {
            val: RValue::Call(
                middle::functions::Analysis(names).into(),
                IndexVec::new(),
                span,
            ),
            ty: Type::BOOL,
...
impl HirFunction for Derivative {
    ...
    fn lower<L: HirLowering>(
        ...
        let val = match self {
            ...
            Self::Partial => {
                let unknown = match ctx.fold.hir[args[1]].contents {
                    ...
                    _ => {
                        ctx.fold.errors.add(DerivativeNotAllowed(ctx.fold.hir[args[1]].span));
                        return None;
                    }
                };
        ...
        FunctionLoweringReturn::Val(TyRValue { val, ty: Type::REAL }).into()
    }
}
...
impl HirFunction for LimFunction {
    ...
    fn allowed_arg_operation(&self, arg: CallArg) -> Option<AllowedOperations> {
        match arg.raw() {
            ...
            1 => Some(AllowedOperations::only(AllowedOperation::UserFunctionReference)),
            _ => None,
        }
    }
        ...
        let (fun, fun_args): (_, IndexVec<_, _>) = match self {
            LimFunction::Native(name) => {
                let args = args.as_raw_slice()[2..]
                    .iter()
                    .filter_map(|x| ctx.lower_expr(*x).map(|val| ctx.rvalue_to_operand(val, span)))
                    .collect();
                (middle::functions::LimFunction::Native(*name), args)
            }
            LimFunction::VerilogA(fun) => {
                let fun = &ctx.fold.hir.functions[*fun];
                let ty = ctx.fold.hir[fun.return_variable].ty;
                if ty != Type::REAL {
                    ctx.fold.errors.add(ExpectedRealFunction { span, function: fun.ident, ty })
                }
                ...
                (middle::functions::LimFunction::VerilogA(cfg), args)
            }
        };

        let fun: middle::functions::LimFunction = fun;

        let call = match ctx.fold.hir[args[0]].contents {
            Expression::BranchAccess(DisciplineAccess::Potential, branch) => {
                let branch = &ctx.fold.hir[branch];
                VoltageLim { voltage: Voltage { lo: branch.lo, hi: branch.hi }, fun }.into()
            }
            Expression::BranchAccess(DisciplineAccess::Flow, branch) => {
                CurrentLim { current: CurrentProbe(branch), fun }.into()
            }
            _ => {
                todo!("Error");
            }
...
impl CfgConversion<DefaultFunctions, LimFunctionDefaultFunctions> for DefaultToLimFun {
    ...
    fn map_call_val(
        ...
        RValue::Call(call, args.into_iter().map(|arg| Self::map_operand(self, arg)).collect(), span)
    }
    ...
    fn map_call_stmnt(
        ...
        StmntKind::Call(
            call,
            args.into_iter().map(|arg| Self::map_operand(self, arg)).collect(),
            span,
        )
    }
}
    ...
    fn lower<L: HirLowering>(
        ...
        let (noise_type, args, name) = match (self, args.as_raw_slice()) {
            (Self::Flicker, [pwr, exp, name @ ..]) => {
                let name = name.get(0).and_then(|name| ctx.fold.eval_string_constant(*name));
                (
            ...
            (Self::White, [pwr, name @ ..]) => {
                let name = name.get(0).and_then(|name| ctx.fold.eval_string_constant(*name));
                (functions::Noise::White, index_vec![ctx.fold_real(*pwr)?], name)
            }
            (Self::Table | Self::TableLog, [input_expr, name @ ..]) => {
                ...
                (noise_type, index_vec![ctx.rvalue_to_operand(input, input_span)], name)
            }
            _ => unreachable!(),
        };
...
impl ParameterizationRead {
    ...
    fn expected_reference(&self, span: Span, refkind: &'static str) -> Error {
        FunctionExpectedReference { span, name: self.symbol(), refkind }
    }
}
...
impl HirFunction for ParameterizationRead {
    ...
    fn lower<L: HirLowering>(
        ...
        let val: DefaultInputs = match self {
            ...
            Self::Simparam => {
                ...
                match args.get(1) {
                    Some(&default) => {
                        ...
                        return FunctionLoweringReturn::Val(TyRValue { val, ty: Type::REAL })
                            .into();
                    }
                    None => SimParam { name, kind: SimParamKind::Real }.into(),
                }
            }
            Self::SimparamStr => {
                let name = ctx.fold.eval_string_constant(args[0])?;
                SimParam { name, kind: SimParamKind::String }.into()
            }
            ...
            Self::ParameterGiven => {
                if let Expression::ParameterReference(param) = ctx.fold.hir[args[0]].contents {
                    ...
                    ctx.fold.errors.add(self.expected_reference(span, "parameter"));
                    return None;
                }
            }
        };
        ...
        FunctionLoweringReturn::Val(TyRValue {
            val: RValue::Use(Spanned { contents: OperandData::Read(val), span }),
            ty,
        })
        .into()
    }
