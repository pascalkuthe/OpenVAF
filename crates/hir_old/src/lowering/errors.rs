...
pub enum Error {
    ...
    TypeMissmatch { span: Span, expected_type: TyPrinter, found: TyPrinter },
    ...
    ExpectedRealFunction { span: Span, function: Ident, ty: Type },

    #[error("user defined $limit function can only have 'input' arguments but '{function}' type but '{arg}' was declared as 'inout'/'output'")]
    UserLimFunctionWithOutputArgs { arg: Ident, function: Ident, arg_decl: Span, call: Span },
    ...
    MultiTypeMissmatch { span: Span, types: ListFormatter<Vec<TyPrinter>>, type_spans: Vec<Span> },
    ...
    WrongFunctionArgCount { expected: usize, found: usize, span: Span },
    ...
    FunctionExpectedReference { span: Span, name: Symbol, refkind: &'static str },

    #[error("{0}")]
    DerivativeError(#[from] middle::derivatives::Error),
...
impl LibraryDiagnostic for Error {
    ...
    fn slices(&self) -> Vec<DiagnosticSlice> {
        match self {
            ...
            Self::TypeMissmatch { span, expected_type, .. } => vec![DiagnosticSlice {
                slice_span: span.data(),
                messages: vec![(
                    AnnotationType::Error,
            ...
            Self::ReferenceTypeMissmatch { expected_type, name, ref_span, decl_span, .. } => vec![
                DiagnosticSlice {
            ...
            Self::DstTypeMissmatch {
                expected_type, name, decl_span, span: assign_span, ..
            } => vec![
                DiagnosticSlice {
            ...
            Self::Recursion { function_name, recursion_span, recursion_traceback } => {
            ...
            Self::MultiTypeMissmatch { span, types, type_spans } => {
                let mut messages = Vec::with_capacity(types.list.len() + 1);
                messages[0] =
                    (AnnotationType::Error, Text::const_str("types mismatched"), span.data());
            ...
            Self::ArgCountMissMatch { missmatch, span, name, decl } => {
            ...
            Self::SimultaneousVoltageAndCurrentProbe { branch, voltage_probes, current_probes } => {
                let mut diagnostics = vec![DiagnosticSlice {
                    slice_span: branch.span.data(),
                    messages: vec![(
                        AnnotationType::Error,
            ...
            Self::UserLimFunctionWithOutputArgs { arg, function, arg_decl, call } => {
                vec![
                    DiagnosticSlice {
