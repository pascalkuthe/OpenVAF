...
use data_structures::{bit_set::BitSet, index_vec::IndexVec, sync::RwLock, HashMap};

use diagnostics::lints::{Lint, LintLevel, Linter};
...
use middle::functions::ParameterCallType;
...
impl<'h, L: HirLowering> HirFold<'h, L> {
    ...
    pub fn handle_builtin_attribute(&mut self, attr: &Attribute, sctx: SyntaxCtx) -> bool {
        let lvl = match attr.ident.name {
            sym::allow => LintLevel::Allow,
            sym::warn => LintLevel::Warn,
            sym::deny => LintLevel::Deny,
            sym::forbid => LintLevel::Forbid,
            _ => return false,
        };
        ...
        let names = if let Some(expr) = attr.value {
            match self.hir[expr].contents {
                ...
                _ => {
                    self.errors
                        .add(ExpectedLintName { attr: attr.ident.name, span: self.hir[expr].span });
                    return true;
                }
            }
        } else {
            ...
            if let Some(lint) = Lint::from_name(&name) {
                if let Some(old) = self.mir.syntax_ctx[sctx].lint_levels.insert(lint, lvl) {
                    Linter::dispatch_late(
                        Box::new(LintLevelOverwrite { span, lint: name, old, new: lvl }),
                        sctx,
                    );
                }
            } else {
    ...
    fn lower_nature(&mut self, id: NatureId, nature: &crate::Nature) -> Nature {
        ...
        let units = self.eval_string_constant(nature.units).unwrap_or(StringLiteral::DUMMY);
    ...
    fn lower_variable(&mut self, id: VariableId, var: &crate::Variable) -> Variable {
        ...
        let default =
            var.default.and_then(|e| self.lower_assign_expr(e, var.ty)).unwrap_or_else(|| {
    ...
    fn lower_parameter(&mut self, id: ParameterId, param: &crate::Parameter) -> Option<Parameter> {
        ...
        let kind = match &param.constraints {
            ...
            crate::ParameterConstraint::Unordered(included, excluded) => {
                let included =
                    included.iter().filter_map(|e| self.lower_assign_expr(*e, param.ty)).collect();
                let excluded =
                    excluded.iter().filter_map(|e| self.lower_assign_expr(*e, param.ty)).collect();
    ...
    pub fn lower_assign_expr(
        ...
        let operand = lctx.rvalue_to_operand(expr, span);
        let cfg = if lctx.cfg_builder.cfg.blocks.iter().all(|x| x.statements.is_empty()) {
        ...
        let operand =
            Operand::new(operand.contents.map_input(|x| x.try_into().unwrap()), operand.span);
    ...
    pub fn eval_string_constant(&mut self, expr: ExpressionId) -> Option<StringLiteral> {
        self.lower_string_expression(expr).map(|x| x.const_eval().unwrap()).and_then(|x| {
    ...
    pub fn eval_real_constant(&mut self, expr: ExpressionId) -> Option<f64> {
        self.lower_real_expression(expr).and_then(|x| x.const_eval()).and_then(|x| {
...
pub fn lower_hir<L: HirLowering>(
    ...
    let net_iter =
        fold.mir.nodes.iter_enumerated().map(|(id, net)| (net.sctx, AttributeCtx::Node(id)));
    ...
    fold.mir.natures =
        src.natures.iter_enumerated().map(|(id, nature)| fold.lower_nature(id, nature)).collect();

    fold.mir.branches =
        src.branches.iter_enumerated().map(|(id, nature)| fold.lower_branch(id, nature)).collect();
    ...
    fold.mir.modules =
        src.modules.iter_enumerated().map(|(id, module)| fold.lower_module(id, module)).collect();
