...
fn is_blacklisted_llvm(llvm_version: &Version) -> Option<&'static str> {
    ...
    let blacklist_var = format!("LLVM_SYS_{}_IGNORE_BLACKLIST", env!("CARGO_PKG_VERSION_MAJOR"));
    ...
    for &(major, minor, patch, reason) in BLACKLIST.iter() {
        let bad_version = Version { major, minor, patch, pre: vec![], build: vec![] };
...
fn is_compatible_llvm(llvm_version: &Version) -> bool {
    ...
    let strict =
        env::var_os(format!("LLVM_SYS_{}_STRICT_VERSIONING", env!("CARGO_PKG_VERSION_MAJOR")))
            .is_some()
...
fn get_llvm_cxxflags() -> String {
    ...
    let no_clean =
        env::var_os(format!("LLVM_SYS_{}_NO_CLEAN_CFLAGS", env!("CARGO_PKG_VERSION_MAJOR")))
            .is_some();
...
fn main() {
    ...
    cc::Build::new().cpp(true).file("wrapper/lld-c.cpp").compile("lldwrapper");
    ...
    let use_debug_msvcrt =
        env::var_os(format!("LLVM_SYS_{}_USE_DEBUG_MSVCRT", env!("CARGO_PKG_VERSION_MAJOR")))
            .is_some();
    ...
    let force_ffi =
        env::var_os(format!("LLVM_SYS_{}_FFI_WORKAROUND", env!("CARGO_PKG_VERSION_MAJOR")))
            .is_some();
    if force_ffi {
