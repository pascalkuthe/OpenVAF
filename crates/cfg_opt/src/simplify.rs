use bitset::BitSet;
use cfg::{smallvec, BasicBlock, BasicBlockData, ControlFlowGraph, Instruction, Op, Terminator};
...
impl<'a> SimplifyTerminators<'a> {
    ...
    fn collapse_goto_chain(
        ...
        // Do not join if the destination contains phi nodes
        if !self.cfg[bb].phis.is_empty() {
            if let Some(chain_end) = chain.pop() {
                bb = chain_end.0
            }
        }
        ...
        if let Some(chain_end) = chain_end {
            if let Some(predeccsor) = predecessor {
                self.update_phis(last, chain_end, predeccsor)
            }
        }
        *start = last;
    }
    ...
    fn update_phis(
        &mut self,
        bb: BasicBlock,
        old_predesssor: BasicBlock,
        new_predecessor: BasicBlock,
    ) {
        for phi in &mut self.cfg[bb].phis {
            let local = if let Some(local) = phi.sources.remove(&old_predesssor) {
                local
            } else {
                let msg = format!("invalid phi {:?}", phi);
                unreachable!("{}", msg)
            };
            phi.sources.insert(new_predecessor, local);
        }
    }
    fn collapse_phis(&mut self, bb: BasicBlock, predesssor: BasicBlock) {
        ...
        let instructions = block.phis.iter().map(|phi| {
            ...
            Instruction { dst: phi.dst.into(), op: Op::Copy, args: smallvec![src.into()], src: 0 }
        });
    ...
    fn run(&mut self) {
        ...
        loop {
            ...
            // GOTO chain from entry can not lead to phi
            self.collapse_goto_chain(&mut start, &mut changed, None);
            ...
            for bb in self.cfg.blocks.keys() {
                ...
                for successor in terminator.successors_mut() {
                    self.collapse_goto_chain(successor, &mut changed, Some(bb));
                }
                ...
                if current != bb {
                    match terminator {
                        Terminator::Goto(dst) => self.update_phis(dst, current, bb),
                        Terminator::Split { true_block, false_block, .. } => {
                            self.update_phis(true_block, current, bb);
                            self.update_phis(false_block, current, bb);
                        }
                        Terminator::Ret => (),
                    }
                }
...
pub fn strip_nops(cfg: &mut ControlFlowGraph) {
    for block in &mut cfg.blocks {
        block.phis.retain(|phi| !phi.sources.is_empty());
