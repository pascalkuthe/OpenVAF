    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match self {
            ...
            LinkerError::PathError(path) => {
                write!(f, "path contains invalid UTF-8 characters: {}", path.display())
            }
        }
    }
}

// TODO support system linker
...
impl Linker for LdLinker {
    fn add_object(&mut self, path: &Path) -> Result<(), LinkerError> {
        let path_str =
            path.to_str().ok_or_else(|| LinkerError::PathError(path.to_owned()))?.to_owned();
        self.args.push(path_str);
        Ok(())
    }

    fn build_shared_object(&mut self, path: &Path) -> Result<(), LinkerError> {
        let path_str = path.to_str().ok_or_else(|| LinkerError::PathError(path.to_owned()))?;
    ...
    fn finalize(&mut self) -> Result<(), LinkerError> {
        lld::link(lld::LldFlavor::Elf, &self.args).ok().map_err(LinkerError::LinkError)
    }
}
...
impl Ld64Linker {
    fn new(target: &spec::Target) -> Self {
        Ld64Linker { args: vec![format!("-arch {}", &target.arch)] }
    }
}
...
impl Linker for Ld64Linker {
    ...
    fn finalize(&mut self) -> Result<(), LinkerError> {
        lld::link(lld::LldFlavor::MachO, &self.args).ok().map_err(LinkerError::LinkError)
    }
}
...
impl Linker for MsvcLinker {
    ...
    fn build_shared_object(&mut self, path: &Path) -> Result<(), LinkerError> {
        let dll_path_str = path.to_str().ok_or_else(|| LinkerError::PathError(path.to_owned()))?;

        let dll_lib_path_str =
            path.to_str().ok_or_else(|| LinkerError::PathError(path.to_owned()))?;
    ...
    fn finalize(&mut self) -> Result<(), LinkerError> {
        lld::link(lld::LldFlavor::Coff, &self.args).ok().map_err(LinkerError::LinkError)
    }
}
