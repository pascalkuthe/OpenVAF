...
use middle::dfa::ResultsVisitor;
use middle::{dfa, impl_pass_span, CfgFunctions, Local, StmntKind};
    ...
    R: Borrow<dfa::GenKillResults<C, ReachingDefinitionsAnalysis<'b, A>>>,
{
    ...
    fn run(self, cfg: &ControlFlowGraph<C>) -> Self::Result {
        let Self { locations, assignments, reaching_definitions } = self;
...
impl<'a, C: CfgFunctions> ResultsVisitor<C> for UseDefBuilder<'a> {
    ...
    fn visit_statement_before_effect(
        ...
        self.visit_rhs(self.locations[block].stmnt_start + id.index(), stmnt.0.read_locals(), state)
    }
