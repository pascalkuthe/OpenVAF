...
use data_structures::bit_set::SparseBitMatrix;
use middle::cfg::{AnalysisPass, BasicBlock, ControlFlowGraph, TerminatorKind};
    ...
    for BuildControlDependenceGraph<I>
{
    ...
    fn run(self, cfg: &ControlFlowGraph<C>) -> Self::Result {
        ...
        for (id, bb) in cfg.blocks.iter_enumerated() {
            ...
            if let TerminatorKind::Split { true_block, false_block, loop_head, .. } =
                bb.terminator().kind
            {
