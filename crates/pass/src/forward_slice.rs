...
use data_structures::bit_set::{BitSet, SparseBitMatrix};
use data_structures::WorkQueue;
use middle::cfg::{
    AnalysisPass, ControlFlowGraph, IntLocation, InternedLocations, LocationKind,
};
use middle::{impl_pass_span, CfgFunctions, Local};
    ...
    C: CfgFunctions,
{
    ...
    fn run(self, _cfg: &ControlFlowGraph<C>) -> Self::Result {
        let Self { tainted_locations, pdg, locations, taint_control_dependencies } = self;
    ...
    A: Borrow<SparseBitMatrix<Local, IntLocation>>,
{
    fn run_impl<C: CfgFunctions, const TAINT_CONTROL_DEPENDENCIES: bool>(
        ...
        let mut work_queue =
            WorkQueue { deque: tainted_locations.iter().collect(), set: tainted_locations };
        ...
        while let Some(loc) = work_queue.take() {
            ...
            if let Some(data_dependencents) = pdg.get_data_dependencies(loc) {
                for data_dependent_loc in data_dependencents.iter() {
                    trace!(location = debug(&locations[data_dependent_loc]), "Data dependent",);
            ...
            if TAINT_CONTROL_DEPENDENCIES && matches!(locations[loc].kind, LocationKind::Terminator)
            {
                    ...
                    pdg.get_control_dependencies(locations[loc].block)
                {
                    for control_dependent_bb in control_dependencents.iter() {
                        ...
                        trace!(
                            locations = debug(
                                tainted_locations.clone().map(|loc| locations[loc]).collect_vec()
                            ),
        ...
        debug!(
            removed_locations =
                debug(removed_locations.iter().map(|loc| locations[loc]).collect_vec()),
