...
fet = CircuitInstance("test_fet", "example_fet", ports=["drain", "gate" ,"gnd"])
fet.set_param("L", "length")
fet.set_param("W", "width")
vdd = CircuitInstance("vdd", "vsource", ports=["vdd", "gnd"])
vdd.set_param("dc", 2)
rf_block = CircuitInstance("rf_block", "inductor", ports=["vdd", "drain"])
